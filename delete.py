...
class BilibiliLotteryCleaner:
    ...
    def get_dynamics(
        ...
        code = _safe_int_code(data.get("code"))
        if code != 0:
            ...
            raise ApiError(code, str(msg), url)
    ...
    def get_lottery_info(self, orig_id: str) -> LotteryQueryResult:
        ...
        try:
            ...
            result = LotteryQueryResult(error_type="rate_limit", message=str(e))
            self._lottery_cache[orig_id] = result
            print(f"   ⚠️  查询抽奖状态被限流，跳过: {orig_id[:18]}")
            return result
        except NetworkError as e:
            result = LotteryQueryResult(error_type="network", message=str(e))
            self._lottery_cache[orig_id] = result
            print(f"   ⚠️  查询抽奖状态失败 (网络错误)，跳过: {orig_id[:18]}")
            return result
        except requests.HTTPError as e:
            result = LotteryQueryResult(error_type="http_error", message=str(e))
            self._lottery_cache[orig_id] = result
            print(f"   ⚠️  查询抽奖状态 HTTP 错误，跳过: {orig_id[:18]}")
            return result
        except Exception as e:
            result = LotteryQueryResult(error_type="unexpected", message=str(e))
            self._lottery_cache[orig_id] = result
            return result
        ...
        try:
            ...
            result = LotteryQueryResult(error_type="json_decode", message=str(e))
            self._lottery_cache[orig_id] = result
            return result
    ...
    def delete_dynamic(self, item: Dict) -> Tuple[bool, str]:
        ...
        if not isinstance(item_params, dict):
            ...
            print(f"   ⚠️  缺少删除参数（params 非 dict），安全跳过: {dyn_id}")
...
def _decrypt_chromium_cookies(
    cookie_db_path: str,
    local_state_path: str,
    prefix_32: bool = False,
) -> Optional[str]:
    """Read and decrypt bilibili cookies from a Chromium cookie database.

    Pure function — no file copying, no process management.
    Testable without browser interaction.

    Returns:
        Cookie header string like ``"DedeUserID=...; bili_jct=..."``,
        or None if required cookies are missing.
    """
    import sqlite3
    ...
    def _decrypt_one(enc_val: bytes, key: bytes) -> str:
        nonce = enc_val[3:15]
    ...
    try:
        ...
        for _host, name, enc_val in cur.fetchall():
            n = name.decode("utf-8")
            if enc_val and enc_val[:3] == b"v10":
                try:
                    cookies_dict[n] = _decrypt_one(enc_val, master_key)
                except Exception:
                    pass
    except Exception:
        return None
...
def _try_chromium_browser(
    ...
    # Early check: are the Windows crypto libs available?
    try:
    ...
    try:
        ...
        cookie_str = _decrypt_chromium_cookies(tmp_name, ls_file, prefix_32)
        if cookie_str:
